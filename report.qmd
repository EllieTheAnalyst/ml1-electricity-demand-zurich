...
The rootogram shows the model fits the 0-2 range well but underestimates days with 5 or more peak hours. The deviance residual plot shows more spread than expected from a well-calibrated Poisson model the visual signature of overdispersion.
...
For the test set (evaluating unseen 2025 data), We prioritized Mean Absolute Error (MAE). MAE provides a direct, highly interpretable metric of real-world performance: exactly how many peak hours per day our forecast is off by. The weekly aggregated predictions track the seasonal pattern well. The model correctly forecasts near-zero peak counts through June-September 2025 and rising counts from October onward. It underestimates the highest winter weeks and overestimates some intermediate periods.
...
The idea behind the GAM was simple: give the model the right amount of freedom for each feature. For complex weather variables like temperature and solar radiation, we let it draw whatever curve the data demanded. For calendar patterns like hour of day and month (which follow known cycles) we kept it constrained. This way the model stays flexible where it needs to be, and disciplined where it should be.

We also gave it memory. By adding lag24 (same hour yesterday) and lag168 (same hour last week), the model learns that electricity demand today is strongly shaped by what happened in the same patterns 24 and 168 hours before.

#### Results

R² (test) = 0.939, RMSE = 3,861 kW, MAE = 2,596 kW — trained on 2022–2024,tested on 2025.

![](outputs/figures/05_gam/05_13_gam_pdp.png)

The partial dependence plots tell the story clearly. Temperature shows a U-shape: demand spikes in the cold (heating), dips in mild weather, and rises again in the heat (cooling). This is exactly what a linear model cannot capture, and exactly why we needed a GAM. Humidity and solar radiation are nearly flat, once temperature and the lag features are in the model, they add almost nothing. lag24 is the steepest curve of all: yesterday's demand at this hour is the single strongest predictor of today's.

![](outputs/figures/05_gam/05_14_gam_diagnostics.png)

The GAM summary confirmed all smooth terms have effective degrees of freedom between 14.8 and 15.8, well above 1, meaning every spline is genuinely non-linear and earning its place. Residuals show no systematic trend, though the ACF reveals autocorrelation at 24h and 48h lags, expected for hourly energy data.

![](outputs/figures/05_gam/05_15_gam_pred_vs_actual.png)

The result speaks for itself: RMSE drops from 7,624 kW (LM) to 3,861 kW, a 49% improvement. Train and test R² differ by only 0.002, confirming the model generalises cleanly to 2025 with no overfitting.
