...
RMSE = 2.61 h/day, MAE = 1.43 h/day, Pseudo-R² = 0.726. Overdispersion ratio = 1.619.
...
The Poisson model assumes variance equals the mean. With an improved dispersion ratio of 1.619, the data has some clustered variance than the model expects—a natural reflection of grid peaks clumping together during extreme weather events **Negative Binomial GLM** would be the technically superior alternative, as it adds a free dispersion parameter that absorbs this extra variance and produces better-calibrated standard errors.
...
This model answers a different question: will this hour be a peak hour? That is a yes or no problem, so we used a Binomial GLM with a logit link, which outputs a probability between 0 and 1 directly interpretable as peak likelihood.

Only about 10% of hours are peak hours, so the default threshold of 0.5 would miss too many. We lowered it to 0.3 because in the Swiss grid, failing to anticipate a peak risks leaving zones without power. Over-preparing is a much safer mistake than under-preparing.

A T:hour_cos interaction was added because the temperature effect on peak probability should be strongest at the times of day when peak hours actually occur.

#### Results

AUC = 0.959, Accuracy = 92%, McFadden R² (train) = 0.525.

![](outputs/figures/04_binomial/04_11_binomial_diagnostics.png)

The calibration plot (on training data) shows the model produces honest probabilities, not just correct labels. Non-peak hours pile up near zero and peak hours spread across higher probabilities, showing the model has genuinely learned to separate the two classes.

![](outputs/figures/04_binomial/04_12_binomial_roc_cm.png)

At a threshold of 0.3, the model achieves a vital 82% Recall for peak events. While it raises more false alarms than a stricter threshold would (Precision: 54%), in a utility context this is the optimal tradeoff: over-preparing to secure grid stability is significantly safer than missing a critical blackout event.

#### Coefficient interpretation

Hour of day dominates, as expected: peaks happen in morning and evening windows. Sunny hours are less likely to be peaks (solar radiation odds ratio below 1). Season matters independently of temperature, reflecting that winter peaks come from heating load and summer peaks from cooling, two different physical mechanisms.

One known limitation: is_weekend causes quasi-complete separation because weekends almost never have peak hours in Zurich (coefficient = -28.47, std_err = 14,699). This does not hurt predictions but makes that coefficient uninterpretable.
...
The GAM was built around one core idea: give the model the right amount of freedom depending on what it is learning. Weather variables like temperature get full flexibility to curve however the data wants. Calendar patterns like hour and month get constrained to known cycles, so the model stays disciplined where the patterns are predictable.

We also gave it memory. Two lag features capture the natural rhythm of electricity consumption: lag24 (same hour yesterday) for the daily cycle, and lag168 (same hour last week) for the weekly contrast between workdays and weekends. This is the main reason the GAM outperforms the LM, which has no memory of past demand at all.

#### Results

R² (test) = 0.939, RMSE = 3,861 kW, MAE = 2,596 kW.

![](outputs/figures/05_gam/05_13_gam_pdp.png)

The partial dependence plots reveal what the model actually learned.**Temperature**, The vertical axis tracks relative changes in demand (higher means more electricity usage). When it is cold (below 10°C), the curve sits high up and slopes downward as weather warms up, representing winter heating shutting off. Between 10°C and 20°C, it forms a smooth, gentle transitional curve as climate conditions moderate. Above 20°C, the spline bends slightly upward, successfully capturing the secondary cooling load from air conditioning during summer heatwaves.
**Humidity and solar radiation**, are nearly flat, meaning once temperature and the lags are in the model, they add little independently. **lag24**, is the steepest curve of all, confirming that yesterday's demand is the strongest predictor of today's. Note that pygam does not center partial dependence terms by default, so the curve shapes matter, not the absolute y-axis values.

![](outputs/figures/05_gam/05_14_gam_diagnostics.png)

The GAM summary confirmed all smooth terms have effective degrees of freedom between 14.8 and 15.8 (well above 1), meaning every spline is genuinely capturing non-linearity. The is_weekend term was penalised to an EDoF of 0 since day of week already contains that information. Residuals show no systematic trend, though the ACF reveals autocorrelation at 24h and 48h lags, which is expected for hourly energy data.

![](outputs/figures/05_gam/05_15_gam_pred_vs_actual.png)

RMSE is 3,861 kW, a 49% improvement. Train and test R² differ by only 0.002, confirming the model generalises cleanly to 2025 with no overfitting.
...
RMSE (test) = 2,661 kW (3.6% of mean demand), MAE = 2,038 kW. Best architecture: (128,64).

![](outputs/figures/07_nn/nn_combined.png)

All three architectures converge in under 120 epochs. The (128,64) network reaches a slightly lower training loss than the other two, but the differences are small and converged values are close. There is no sign of instability or oscillation in any of the curves, which means the Adam default learning rate is appropriate for this dataset. Early stopping terminates training before any architecture reaches 1000 iterations, suggesting that overfitting would become a concern with more epochs but is well controlled here.
...
| Model | RMSE (kW) | MAE (kW) | R² train | R² test |
|---|---|---|---|---|
| LM | 7,624 | 6,126 | 0.786 | 0.752 |
| GAM | 3,861 | 2,596 | 0.937 | 0.939 |
| NN (128,64) | 2,661 | 2,038 | 0.975 | 0.971 |
| SVM | 881 | 654 | 0.997 | 0.997 |

**Classification and count models**

| Model | Task | Metric | Value |
|---|---|---|---|
| GLM Poisson | peak hours/day (count) | RMSE | 2.61 h/day |
| GLM Poisson | | R² | 0.69 |
| GLM Binomial | is_peak (binary) | AUC | 0.959 |
| GLM Binomial | | Accuracy | 92% |

The four regression models show a clear progression: each step up in complexity reduces test RMSE. The LM establishes a baseline at 7,624 kW. The GAM's non-linear splines reduce RMSE by 49%, with most of the gain coming from the temperature curve. The biggest jump comes with the SVM and NN, which reduce RMSE by 88% and 65% respectively over the LM, capturing non-linear interactions between temperature, hour, and season that additive models cannot represent.

> The Neural Network explains 97.1% of variance in hourly demand. With a mean load of approximately 74,000 kW, its RMSE of 2,661 kW corresponds to roughly 3.6% of mean demand, a strong result for operational grid planning at EWZ.
...
The GLM Binomial stands out as the clearest practical result. An AUC of 0.959 using only weather and time variables means peak hours are largely predictable from publicly available data well in advance, with direct relevance for grid operations.
