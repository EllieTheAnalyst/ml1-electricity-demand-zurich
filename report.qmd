...
The partial dependence plots reveal what the model actually learned. Temperature shows a clear U-shape: demand spikes in the cold (heating), reaches its minimum around 20°C, and rises again in the heat (cooling). This is the result that justifies the GAM over the LM. A linear model draws one straight line through this and gets it wrong at both ends. Humidity and solar radiation are nearly flat, meaning once temperature and the lags are in the model, they add little independently. lag24 is the steepest curve of all, confirming that yesterday's demand is the strongest predictor of today's. Note that pygam does not center partial dependence terms by default, so the curve shapes matter, not the absolute y-axis values.
...
RMSE dropped from 7,624 kW (LM) to 3,861 kW, a 49% improvement. Train and test R² differ by only 0.002, confirming the model generalises cleanly to 2025 with no overfitting.
